...
function PostCard({
  ...
  return (
    /* ДВЕ КОЛОНКИ НА ШИРОКОМ ЭКРАНЕ: коллаж слева, текст справа.
       ...
       ─── ПОЧЕМУ ПОТОЛОК ШИРИНЫ КОЛЛАЖА ТАКОЙ БОЛЬШОЙ ───

        Размер коллажа считается так: высота упирается в --collage-h,
        ширина получается из высоты и пропорций ряда. Пока в потолок
        ширины никто не упирается, ВСЕ посты ленты выходят ОДНОЙ ВЫСОТЫ —
        а это и есть тот ритм, из-за которого лента читается как лента, а
        не как набор разных блоков.

        При прежних 56% в потолок упирался всякий широкий коллаж: пост из
        трёх квадратных кадров получал 754×503 вместо 883×589 и оказывался
        на 86px ниже соседей. Теперь потолок поднят настолько, чтобы его
        хватало любой разумной раскладке, и высота у всех одна.

        Ширина при этом у постов всё равно разная, и иначе не будет:
        кадры не обрезаются, поэтому пропорции коллажа задают сами
        фотографии. Три вертикальных 9:16 физически у́же трёх квадратов
       при одинаковой высоте. */
    <article className="flex flex-col gap-7 lg:flex-row lg:items-start lg:gap-10 lg:[--collage-w:68%] xl:[--collage-w:70%]">
