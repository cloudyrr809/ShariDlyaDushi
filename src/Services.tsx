...
/* КАРТОЧКА УСЛУГИ — журнальный разворот, а не коробка. Плиты нет,
   содержимое лежит прямо на фоне страницы; структуру держат волосяные
   линии и типографика, а не заливки.

   ВЫСОТА ТЕЛА ЗАДАНА ЯВНО — 32rem. Считается так: 73 шапка + 63 вкладки
   оставляют 764px видимого экрана при окне 900, из них заголовок ~98 и
   строка характеристик ~76. Побочная выгода: все восемь услуг одной
   высоты, и переключение вкладок не дёргает страницу. Длинное описание
   прокручивается внутри своей колонки.

   Фотографии — коллаж, а не карусель: крупный кадр на две трети ширины и
   два поменьше стопкой справа, любой открывается в общем просмотрщике.
   Четвёртый и далее прячутся под плашку «+N». */
...
const ServiceCard = ({
  ...
  return (
    <article className="relative">
      ...
      {/* Строка с разделителями-волосинками, а не три плитки: плитки были
          рамкой в рамке и занимали 116px против 76.

          На телефоне делить строку на три негде — при 390px на колонку
          осталось бы 106px, и «Длительность» обрезалась бы. */}
      ...
      <div className="mt-7 grid gap-6 lg:h-[32rem] lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1fr)] lg:gap-10">
        ...
        <div className="flex min-h-0 flex-col">
          ...
          {/* Без подложки и рамки: черта сверху заодно превращает свободное
              место над ней в воздух колонки, а не в дыру. */}
...
/* НАШ ПОДХОД — та же манера, что у шагов «как получить скидку» на Акциях:
   свой оттенок, скругление, тень, вертикальный сдвиг и шарик над кромкой.
   Узор свой: углы в обратную сторону, оттенки в другом порядке, шарики те
   три, что не заняты на Акциях. */
const approachCards = [
  {
    title: "Под ключ",
...
export default function Services() {
  ...
  return (
    <div className="bg-[#FDFBFD] text-[#2D2433] overflow-hidden relative scroll-smooth">
      ...
      {/* Вкладки — общий компонент с «Каталогом». gap-6 — это МИНИМАЛЬНЫЙ
          просвет: на широком экране justify-between всё равно раздвигает
          вкладки, а на 1280px восьми пунктам при gap-8 не хватало 33px и
          строка начинала прокручиваться. */}
      ...
      {/* Контейнер max-w-[79rem] px-6 — тот же, что у шапки, панели вкладок
          и карточки услуги: иначе края блока разъедутся со страницей. */}
      ...
      <section className="relative z-10 w-full pt-14 pb-24 md:pt-20 md:pb-32">
        <div className="mx-auto w-full max-w-[79rem] px-6">
          ...
          {/* У рукописного «д» росчерк уходит на 0.867em ниже базовой линии,
            а шрифт объявляет себе descent всего 0.2em — при leading-none
            хвост торчал за пределы строки и ложился на заголовок. pb в em,
            а не в px, чтобы работало и на мобильном кегле.

            -translate-x — оптическая поправка: по метрикам подпись
            отцентрована идеально, но росчерк добавляет веса справа. */}
