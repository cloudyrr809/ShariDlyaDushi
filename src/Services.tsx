...
export default function Services() {
  ...
  return (
    <div className="bg-[#FDFBFD] text-[#2D2433] overflow-hidden relative scroll-smooth">
      ...
      {/* ШАПКА СТРАНИЦЫ.
          Плашка вместо прежнего фотофона: заголовок читается сам по себе,
          без вуалей, блюра и теней под буквами.

          Фон — почти незаметный градиент из холодного сиреневого в холодный
          серый, вместо прежнего тёплого пудрово-розового #FDF2F5. Слева
          purple-50 стыкуется с тёплой белизной страницы мягче, чем чистый
          slate-50, а вправо уходит в нейтраль — под фотографию.

          Тёмный текст здесь #2D2433 (баклажановый), а не чёрный и не серый.

          Секция позиционированная и идёт в разметке после цветных пятен
          (blobs) выше, поэтому перекрывает их — фон остаётся чистым. */}
      <section className="relative overflow-hidden bg-gradient-to-r from-purple-50 to-slate-50 py-12 md:py-16">
        {/* ФОНОВОЕ СВЕЧЕНИЕ. Два размытых круга вместо плоской заливки:
            фон перестаёт быть однородным и слегка «дышит» цветом.
            overflow-hidden у секции обрезает их по краям блока, поэтому
            свечение выглядит вписанным, а не наклеенным сверху.
            aria-hidden + pointer-events-none: чисто декоративные, они не
            должны попадать в озвучку и перехватывать курсор. */}
        <div
          aria-hidden="true"
        ...
        {/* Тот же контейнер, что у панели вкладок и карточек ниже, поэтому
            левый край крошек, заголовка и подзаголовка стоит ровно над левым
            краем контента страницы. Высота не фиксированная: в блоке строки
            разной длины и картинка, жёсткая h-48/h-64 либо резала бы
            содержимое, либо оставляла пустоту.
            z-10 — поверх кругов свечения. */}
        <div className="relative z-10 mx-auto w-full max-w-[79rem] px-6">
          {/* items-start, а не items-center. При центрировании текстовая
              ...
              Услугах в три, поэтому крошки стояли на 162px против 175px —
          ...
          <div className="grid grid-cols-1 items-start gap-8 md:grid-cols-2">
            ...
            <div>
          {/* Хлебные крошки. «Главная» — рабочая ссылка, текущий раздел —
              обычный текст с aria-current, как и положено крошкам. */}
          <nav aria-label="Хлебные крошки">
            {/* Иерархия: пройденный путь приглушён, текущий раздел выделен
                цветом заголовка и полужирным.

                «Главная» намеренно НЕ настолько бледная, как text-gray-400:
                это рабочая ссылка, а на розовом фоне gray-400 даёт около
                2.5 при норме 4.5. Взят самый светлый тон, который норму
                проходит, — рядом с полужирным фиолетовым «Услуги» он всё
                равно читается как приглушённый. Разделитель декоративный
                (aria-hidden), к нему требования по контрасту не применяются,
                поэтому он светлее всех. */}
            <ol className="flex items-center gap-2 text-sm">
              ...
              <li aria-current="page" className="font-medium text-[#513A6B]">
                Услуги
              </li>
            </ol>
          </nav>
          ...
          <p className="mt-5 max-w-2xl text-[15px] leading-relaxed font-normal text-[#5A4D66] md:mt-6 md:text-base">
            Мы подходим к каждому проекту с особым вниманием, создавая арочные
            украшения, фигуры и цветочные композиции, чтобы сделать ваше
            мероприятие незабываемым.
          </p>
            </div>

            {/* ПРАВАЯ КОЛОНКА — снимок, который уравновешивает текст.
                Пока стоит существующее фото из проекта: живая картинка
                читается лучше однотонной заглушки, а поменять её — это одна
                строка src. object-cover, поэтому кадр любых пропорций
                заполнит скруглённый бокс без искажений. */}
            {/* Скругление, тень и обрезка — на обёртке, увеличение — на самой
                картинке. Иначе при наведении она вылезла бы за скруглённый
                угол прямоугольником. Тот же приём, что у композиций на
                главной. group — чтобы наведение ловилось всей плашкой. */}
            <div className="group h-48 w-full overflow-hidden rounded-3xl shadow-sm md:h-64">
              <img
                ...
                // Не lazy: картинка видна сразу при открытии страницы и она
                // здесь самая крупная. loading="lazy" отложил бы её до
                // попадания в вьюпорт, то есть до уже случившейся отрисовки,
                // и главный кадр страницы приезжал бы с задержкой.
                fetchPriority="high"
                decoding="async"
