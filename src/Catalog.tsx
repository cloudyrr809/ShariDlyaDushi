...
const ProductCard = ({
  ...
  return (
    /* Нажатие по самой карточке открывает подробности — так же, как
       кнопка «подробнее». Обработчик стоит на карточке, а кнопки внизу
       останавливают всплытие: иначе «в корзину» заодно открывало бы
...
export default function Catalog() {
  ...
  /* Какая композиция открыта подробно. null — окна нет. */
  ...
  return (
    <div className="bg-[#FDFBFD] text-[#2D2433] scroll-smooth">
      ...
      */}
      ...
      {/* Подробности о композиции. Окно одно на всю страницу, а не по
          одному на карточку: сорок скрытых модалок в разметке — сорок
          лишних поддеревьев, из которых видно максимум одно. */}
      <ProductDialog product={details} onClose={() => setDetails(null)} />
