...
/** viewBox обрезан по фактическим границам рисунка: в исходном кадре 24×24
 *  логотип занимает только полосу y 7..19, и при одинаковом h-4 он выходил
 *  вдвое мельче Instagram. */
const VkIcon = ({
...
/** Небо за курсором, в пикселях. Знак отрицательный — фон едет ПРОТИВ
 *  курсора: встречное движение удваивает разницу планов. Величина меньше,
 *  чем у шаров, иначе параллакс выворачивается наизнанку. */
const SKY_MOUSE_X = -9;
const SKY_MOUSE_Y = -6;
...
type Cloud = {
  ...
  stagger: number;
  float: number;
  /* Цветокоррекция у каждого облака своя: они лежат на z-10, то есть НАД
     фоновым слоем, и не получают ни его saturate, ни вуали.
...
/* ОБЛЕГЧЁННЫЙ РЕЖИМ ПЕРВОГО ЭКРАНА (сенсорные устройства).

   На телефоне экран шёл 2-5 кадров в секунду. Дело не в JS, а в числе
   живых слоёв: 17 шаров с drop-shadow по альфа-каналу, SVG с 17
   ленточками, небо с blur(3px) и зерном на mix-blend-mode, два облака с
   размытием и маской.

   Заодно чинится вторая беда: во встроенном браузере Телеграма шары были
   прозрачными. Прозрачность считается из прокрутки (op = 1 − fly), а fly —
   от высоты экрана в svh; этот браузер меряет высоту по-своему.
...
export const Hero = () => {
  ...
  useEffect(() => {
    ...
    /* Событие прокрутки только БУДИТ цикл. Само положение он читает сам,
       в каждом кадре, — почему именно так, расписано в tick ниже. */
    ...
    const tick = (now: number) => {
      ...
      /* ═══ ПРОГРЕСС ПРОКРУТКИ ЧИТАЕМ КАЖДЫЙ КАДР И БЕЗ СГЛАЖИВАНИЯ ═══

         Так было не всегда: положение бралось из события scroll, а потом
         t догонял цель с коэффициентом 0.12 за кадр. Это примерно восемь
         кадров до схождения, то есть гроздь отставала от страницы на
         130 мс.

         При медленной прокрутке отставание незаметно — шары «красиво
         улетают». А при быстром пролистывании страница уезжает сразу, а
         гроздь ещё догоняет прежнее место: получается смазанность, будто
         не хватает кадров. Жалоба звучала как «маленький ФПС», хотя
         кадров хватало — просто картинка отставала от пальца.

         Плюс на телефоне события scroll во время инерции приходят реже
         кадров, и цель обновлялась рывками.

         Теперь положение берём прямо из геометрии в начале каждого кадра —
         гроздь привязана к странице точно. Чтение rect в начале кадра, ДО
         единой записи стиля, не вызывает лишнего пересчёта раскладки: она
         на этот момент и так актуальна.

         Прогресс считаем ПО СОБСТВЕННОМУ ПОЛОЖЕНИЮ первого экрана, а не по
         window.scrollY и высоте: во встроенном браузере Телеграма высота в
         svh меряется по-своему, и от старой формулы шары выходили
         прозрачными. top и height одного элемента всегда согласованы между
         собой, как бы браузер ни понимал svh. */
      const hero = heroRef.current;
      if (hero) {
        const r = hero.getBoundingClientRect();
        a.t = r.height > 0 ? clamp01(-r.top / (r.height * 0.9)) : 0;
      }

      // Курсор по-прежнему со сглаживанием: это ручной ввод, и мягкость
      // здесь — часть замысла, а не отставание от страницы.
      ...
      /* «Дыхание» считаем здесь и подмешиваем в тот же transform, который
         и так пишется каждый кадр. Отдельные CSS-кейфреймы на обёртках
  ...
  return (
    ...
    >
      ...
      {/* ФОН — один движущийся слой (skyRef): и параллакс, и «дыхание» пишет
          в него JS одним transform.

          Зерно лежит ВНУТРИ слоя: у него mix-blend-mode, и снаружи
          смешение пересчитывалось бы по всему экрану каждый кадр.
          -inset-[4%] — чтобы при сдвиге не показался край кадра.
          Градиенты снаружи: они неподвижны. */}
      ...
      {/* ОБЛАКА. На телефоне их нет: каждое — размытие плюс радиальная маска,
          два самых дорогих эффекта сразу, а глубина ради которой они и
          задуманы читается только на широком кадре с параллаксом. */}
      {!lite &&
        CLOUDS.map((c, i) => (
          <div
            key={`cloud-${i}`}
      ...
      {/* ГРОЗДЬ ШАРИКОВ. Смещена вправо, слева освобождается колонка под
          заголовок. zIndex 20 — выше заголовка (15) и ниже углов (30):
          левые шары ложатся ПОВЕРХ хвоста «ШАРАХ», но углы остаются
          сверху, иначе шар перекрыл бы ссылки.

          ПРИВЯЗКА на десктопе — не в процентах от экрана, а на том же
          расстоянии от центра, что и сетка контента: заголовок растёт от
          края сетки, и в процентах наезд то съедал бы букву целиком, то
          пропадал. Поэтому же тут нет -translate-x-1/2 — слева фиксируется
          край грозди, а не середина.
              ...
              <img
                ...
                style={{
                  ...
                  /* ГРАДАЦИЯ ПО ГЛУБИНЕ + ТЕНЬ.
                     ...
                     При одной яркости на все семнадцать шаров соседние
                     розовый и сиреневый давали контраст 1.15 при 1.0 =
                     «неотличимы»: они различались тоном, но не светлотой, и
                     гроздь читалась одним пятном. Теперь яркость и
                     насыщенность раздаются по depth (0.55 у дальних, 1.05 у
                     передних).

                     Тень — drop-shadow, то есть по альфа-каналу: повторяет
                     силуэт и отделяет шар даже там, где два соседа совпали
                     по светлоте. Лишнего слоя не создаёт — фильтр у картинки
                     уже был. */
                  filter: (() => {
                    const t = (b.depth - 0.55) / 0.5; // 0 у дальних, 1 у передних
                    /* Дальние НЕ затемняем: при brightness 0.74 шар выходил
                       на L60 против неба L69 — темнее фона, то есть читался
                       как грязь. В воздушной перспективе дальнее выцветает,
                       оставаясь светлым, поэтому по глубине едет saturate. */
                    /* Диапазоны намеренно узкие: выцветание вместе с
                       прозрачностью давало «растворение» — сквозь дальний шар
                       просвечивало небо. Глубину держит тень, а не
                       прозрачность. */
      ...
      {/* Заголовок и ссылка одной левой колонкой: общая выключка не
        разъезжается при смене кегля.

        z-[15] на десктопе — НИЖЕ грозди (z-20), отсюда глубина: передние
        шары наезжают на хвост «ШАРАХ». На мобильном z-30, там гроздь
        приглушена и текст должен читаться поверх неё.

        px-6 + max-w-[76rem] — та же пара, что в Header и во всех секциях:
        менять ширину в одном месте, не поменяв в остальных, нельзя. */}
      <div className="pointer-events-none absolute inset-0 z-30 flex items-center px-6 select-none lg:z-[15]">
        <div className="mx-auto w-full max-w-[76rem]">
          <div className="text-center lg:max-w-[56rem] lg:text-left">
            {/* Заголовок: Montserrat 800, не 900 — у Black просветы внутри
              букв схлопываются на крупном кегле. Отрицательный трекинг
              обязателен: капс Montserrat набран с запасом под строчные.

              -ml — ОПТИЧЕСКАЯ ВЫКЛЮЧКА, и значения у строк РАЗНЫЕ. Замерено
              канвасом по загруженному шрифту: «Д» даёт отступ 0.0125em,
              «В» — 0.0688em, разница почти в шесть раз. Одинаковая
              компенсация уводила строки в разные стороны. Единица em, чтобы
              переезжала вместе с кеглем.
            ...
            {/* Ссылка текстом, а не кнопка с заливкой: плашка рядом с
                десятисантиметровым «ДУША В ШАРАХ» смотрелась бы наклейкой, да
                и в шапке уже есть одна залитая. Черта стоит сразу — она и есть
                признак, что сюда можно нажать. */}
            <Link
              to="/catalog"
      ...
      {/* УГЛОВАЯ РАМКА: два ряда на той же сетке max-w-[76rem], то есть по
        краям логотипа и кнопки в шапке. Без плашек и теней — рамку держит
        позиция. z-30, всегда над гроздью, иначе шар перекрыл бы ссылки. */}
