      ...
      {flights.map((f) => {
        ...
        return (
          ...
          <div
            ...
            /* z-[45] — выше ЛЮБОГО содержимого страниц (там максимум z-40)
               и ниже шапки (z-50), окон и корзины (z-50 и z-100).

               Было z-20 — ровно столько же, сколько у панели вкладок в
               каталоге и услугах. При равном слое выигрывает тот, кто ниже
               по разметке, а шарики подключены в main.tsx до содержимого:
               тонкая черта под вкладками проезжала прямо по летящему
               шарику, будто он за стеклом.

               Под шапку шарик по-прежнему уходит, и так и надо: она
               непрозрачная и лежит поверх всего сайта. */
            className={`pointer-events-none fixed inset-y-0 z-[45] hidden min-[1420px]:block ${
