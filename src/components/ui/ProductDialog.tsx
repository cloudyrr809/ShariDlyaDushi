    ...
    >
      <div
        ...
        /* На телефоне окно занимает почти весь экран и прижато к низу —
           ...
           по центру, и там всё остаётся как было.

           ПОЧЕМУ НА ТЕЛЕФОНЕ h-[92dvh], А НЕ max-h-[92svh].

           Две причины, и обе про «окно открывается то большим, то
           поменьше».

           1. svh — это высота экрана при РАЗВЁРНУТОЙ панели браузера,
              самая маленькая из возможных. Панель то показывается, то
              прячется при прокрутке (в Телеграме и Chrome одинаково), а
              92svh при этом не меняются — меняется доля видимого экрана,
              которую они занимают. У верха страницы лист выглядел почти
              во весь экран, после прокрутки — заметно ниже видимой
              области. dvh считает от того, что видно СЕЙЧАС, а пока окно
              открыто, прокрутка страницы заблокирована и панель уже никуда
              не денется — значит и высота за время показа не дрогнет.

           2. max-h — это потолок, а не размер: высоту задавало
              содержимое. У композиции с коротким описанием лист выходил
              ниже, чем у длинного. Фиксированная высота убирает и эту
              зависимость: окно всегда одно и то же. */
        className="flex h-[92dvh] w-full max-w-4xl flex-col overflow-hidden rounded-t-3xl bg-white shadow-[0_30px_80px_-20px_rgba(45,36,56,0.45)] sm:h-auto sm:max-h-[88svh] sm:rounded-3xl"
      >
